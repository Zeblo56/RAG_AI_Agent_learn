...
def check_md5(md5_str:str):
    '''检查被传入的md5字符串是否已经被处理过了
        return False(md5未处理） True(已经处理过，已有记录)
        '''
    if not os.path.exists(config.md5_path):
        #if进入表示文件不存在，那肯定没有处理过这个md5
        open(config.md5_path, 'w',encoding='utf-8').close()  #自动创建
        return False
    else:
...
class KnowledgeBaseService(object):
    def __init__(self):
        ...
        )   #记录向量存储的实例 即Chroma向量库对象

        self.spliter = RecursiveCharacterTextSplitter(
            chunk_size=config.CHUNK_SIZE, #表示分割后的文本段最大长度
            chunk_overlap=config.CHUNK_OVERLAP_SIZE, #表示连续文本段允许的字符重叠数量
            separators=config.SEPARATORS, #表示自然段落划分的符号
